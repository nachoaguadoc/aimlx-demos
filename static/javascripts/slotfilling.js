	...
	for (var l in labels_model) {
		...
		for (i = 0; i < l.length; i++) {
			if (l[i] == "O") {
				...
				formatted_text += '<span class="blue">' + original_splitted[i] + ' </span>' + '<span style="color:Orchid">' + ' (' +  l[i].toLowerCase()  + ') ' + ' </span>';
			}
		}
...
function submit(input_text) {
	...
	url = "/slotfilling";
	var data = {"input": input_text.toLowerCase()};
	$.ajax({
	  type: "POST",
	  url: url,
...
$(document).ready(function(){
	...
	$.getJSON("../static/javascripts/lists/slotfilling.json", function(json) {
