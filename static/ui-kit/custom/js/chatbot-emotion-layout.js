var ChatbotLayout = {
    ...
    pushMessage: function (messageData, type, emotion) {
        switch (type) {
            case 'client':
            ...
            case 'bot':
                if (this.isLoading) {
                    this.updateBotSpeechBuble(messageData, emotion);
                } else {
                    this.addBotSpeechBuble(messageData, emotion);
                }
                break;
            default:
                this.addBotSpeechBuble(messageData, emotion);
    ...
    updateBotSpeechBuble: function (messageData, emotion) {
        this.isLoading = false;
        $('.sample-box--selected').removeClass('sample-box--selected');
        $('#chat-data .speech-buble').last().empty().append(messageData);
        $('#chat-data img').last().attr("src",this.getImageEmotion(emotion));
        $('#input-submit').prop('disabled', false);
    },
    getImageEmotion: function (emotion) {
        var src_im = '';
        switch (emotion) {
            case 'smile': src_im = "../static/ui-kit/custom/assets/bot_smile.svg";
                break;
            case 'angry': src_im = "../static/ui-kit/custom/assets/bot_angry.svg";
                break;
            case 'no_answer': src_im = "../static/ui-kit/custom/assets/bot_no_answer.svg";
                break;
            default: src_im = "../static/ui-kit/custom/assets/bot_smile.svg";
        }
        return src_im
    },
    addBotSpeechBuble: function (messageData, emotion) {
        var src_im = this.getImageEmotion(emotion);
        $('#chat-data').append('<div class="bot-buble-container"><img src=' + src_im
            + '/><div class="speech-buble">' + messageData + '</div></div>');
    },
